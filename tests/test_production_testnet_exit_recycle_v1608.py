...
class BalanceBybit(FakeBybit):
    ...
    def fetch_balance(self):
        ...
        return {
            "total": dict(self.balance_total),
            "free": dict(self.balance_free),
        }
        ...
        return {
            ...
            "performance": {
                ...
                "realized_net_after_dust_usd": (
                    self.realized_pnl_usd - self.dust_cost_basis_usd
                ),
            },
        }
    ...
    def mirror_events(self, events):
        ...
        for event in events:
            ...
            if event["side"] == "buy":
                ...
                continue
            if self._terminal_first and len(
                [row for row in self.events if row["side"] == "sell"]
            ) == 1:
                output.append(
                    {
                        "status": "canceled",
...
def test_protective_sells_do_not_consume_buy_entry_budget(tmp_path):
    ...
    second_buy = {
        **buy_event(),
        "timestamp": "2026-08-24T20:00:02+00:00",
        "event_id": "entry-2",
        "symbol": "ETH/USDT",
    }
...
def test_non_tradeable_dust_closes_executor_risk_and_recycles_fast_slot(tmp_path):
    ...
    assert health["performance"]["closed_positions"] == 1
    ...
    event = lane._new_event(
        ...
        quantity=0.00002,
        price=100.0,
        reason="fast_collective_testnet_exit:dust",
        now=1_000.0,
        remaining_quantity=0.0,
    )
    pending = {
