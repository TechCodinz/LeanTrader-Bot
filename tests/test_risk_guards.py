...
def test_no_breach():
    ...
    lim = RiskLimits(
        max_loss_per_symbol=0.5, max_daily_loss=0.5, max_account_drawdown=0.9, pct=True
    )
...
def test_per_symbol_breach():
    ...
    lim = RiskLimits(
        max_loss_per_symbol=0.02, max_daily_loss=0.5, max_account_drawdown=0.9, pct=True
    )  # 2% = $20
    halt, reasons = should_halt_trading(st, lim)
    assert halt is True
...
def test_daily_loss_breach():
    ...
    lim = RiskLimits(
        max_loss_per_symbol=0.9, max_daily_loss=0.02, max_account_drawdown=0.9, pct=True
    )  # 2% = $20
    halt, reasons = should_halt_trading(st, lim)
    assert halt is True
...
def test_drawdown_breach():
    ...
    lim = RiskLimits(
        max_loss_per_symbol=0.9, max_daily_loss=0.9, max_account_drawdown=0.10, pct=True
    )  # 10%
    halt, reasons = should_halt_trading(st, lim)
    assert halt is True
