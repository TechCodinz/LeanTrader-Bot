...
def test_promotion_from_paper():
    st = RampState(
        ...
        stats={"days": 15, "drawdown": 0.01, "slippage_bps_p90": 10.0, "reject_rate": 0.01},
    )
...
def test_no_promotion_due_to_slippage():
    st = RampState(
        mode=Mode.PAPER,
        stats={"days": 15, "drawdown": 0.01, "slippage_bps_p90": 40.0, "reject_rate": 0.01},
    )
    pol = RampPolicy()
...
def test_rollback_from_tiny():
    st = RampState(
        mode=Mode.TINY_PROD,
        stats={"days": 5, "drawdown": 0.05, "slippage_bps_p90": 10.0, "reject_rate": 0.01},
    )
