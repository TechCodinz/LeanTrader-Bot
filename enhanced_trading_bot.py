    ...
    def initialize_database(self):
        ...
        try:
            ...
            tables = [
                ...
                (
                    'bybit_trades',
                    'CREATE TABLE IF NOT EXISTS bybit_trades (id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT NOT NULL, side TEXT NOT NULL, amount REAL NOT NULL, entry_price REAL NOT NULL, current_price REAL, unrealized_pnl REAL, status TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)',
                ),
            ]
        ...
        try:
            if data.startswith("trade_"):
                ...
                await query.edit_message_text(
                    f"✅ Trade executed: {symbol} {side} (Confidence: {confidence}%)"
                )
                ...
                chart_url = (
                    f"https://www.tradingview.com/chart/?symbol=BYBIT:{symbol.replace('/', '')}"
                )
...
🚀 ENHANCED TRADING BOT"""
                        ...
                        logger.info(
                            f"📊 {pair}: ${price_data['price']:,.2f} - {signal['action']} ({signal['confidence']:.1f}%)"
                        )
