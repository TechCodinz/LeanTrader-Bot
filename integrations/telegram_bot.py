...
def _send_text(text: str, chat_id: str) -> bool:
    ...
    try:
        ...
        if r.status_code == 429:
            retry_after = (
                r.json().get("parameters", {}).get("retry_after")
...
def send_signal(text: str, *, vip: bool = False) -> bool:
    chat_id = (
        os.getenv("TELEGRAM_VIP_CHAT_ID") if vip else os.getenv("TELEGRAM_FREE_CHAT_ID")
    ) or (TELEGRAM_VIP_CHAT_ID if vip else TELEGRAM_FREE_CHAT_ID)
