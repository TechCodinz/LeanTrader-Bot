...
try:
    ...
    try:
        if not os.path.exists('runtime/ultra_scan.json'):
            with open('runtime/ultra_scan.json','w',encoding='utf-8') as fh: json.dump({}, fh)
        if not os.path.exists('runtime/ultra_ops.json'):
            with open('runtime/ultra_ops.json','w',encoding='utf-8') as fh: json.dump([], fh)
        if not os.path.exists('runtime/ultra_plans.json'):
            with open('runtime/ultra_plans.json','w',encoding='utf-8') as fh: json.dump([], fh)
    except Exception:
        pass
