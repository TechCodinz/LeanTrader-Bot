...
def find_py_files(root: str, exclude_dirs=None) -> List[str]:
    if exclude_dirs is None:
        exclude_dirs = {".git", "__pycache__", "venv", "env", ".venv", "node_modules", "reports"}
    out = []
...
def analyze_project(root: str) -> Dict[str, Any]:
    ...
    report: Dict[str, Any] = {"root": os.path.abspath(root), "files": {}, "summary": {"files": len(files), "parse_errors": 0, "missing_imports_total": 0}}
    for p in files:
