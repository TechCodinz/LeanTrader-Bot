...
def main() -> int:
    ...
    p.add_argument(
        "--write-json", default=os.getenv("MEMPOOL_RISK_PATH", "runtime/mempool_risk.json")
    )
    args = p.parse_args()
    ...
    def _cb(tx: Dict[str, Any]) -> None:
        ev = mon.observe(tx)
        try:
            MEMPOOL_RISK.labels(symbol=args.symbol.upper(), timeframe=args.timeframe.upper()).set(
                mon.current_risk
            )
        except Exception:
            pass
