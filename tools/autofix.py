    ...
    for f in files:
        ...
        if (
            ...
            issues.append(
                {
                    "file": str(f),
                    "issue": "direct self.router=router without RouterAdapter",
                }
            )
        ...
        if re.search(r"\bcreate_order\s*\(", txt) and "safe_place_order" not in txt:
            issues.append(
                {
                    "file": str(f),
                    "issue": "uses create_order; consider safe_place_order wrapper",
                }
            )
...
def auto_wrap_router(files):
    ...
    for f in files:
        ...
        new_txt = new_txt.replace(
            "self.router = router",
            "self.router = RouterAdapter(router, logger=getattr(self, 'logger', None))",
        )
        if new_txt != txt:
            # backup original
...
def main():
    ...
    ap.add_argument(
        "--apply",
        action="store_true",
        help="Run fixers (black/isort/ruff/autoflake) if available",
    )
    ap.add_argument(
        "--auto-wrap-router",
        action="store_true",
        help="Auto-replace `self.router = router` with RouterAdapter wrapping (creates .bak files)",
    )
    ap.add_argument(
        "--run-smike",
        action="store_true",
        help="Locate and run smike.py (or smoke.py) automatically and capture output",
    )
    ap.add_argument(
        "--full-scan",
        action="store_true",
    ...
    if args.full_scan:
        ...
        if full_scan_res.get("router"):
            ...
            if rinfo.get("methods"):
                print(
                    "Router methods presence sample:",
                    {k: v for k, v in list(rinfo["methods"].items())[:6]},
                )
