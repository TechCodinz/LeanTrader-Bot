...
class MarketDataManager:
    ...
    def fetch_ohlcv(
        self, symbol: str, timeframe: str = '5m', limit: int = 500, exchange: Optional[str] = None
    ) -> pd.DataFrame:
    ...
    def _fetch_fresh_ohlcv(
        self, symbol: str, timeframe: str, limit: int, exchange: Optional[str] = None
    ) -> pd.DataFrame:
    ...
    def save_for_training(self, symbol: str, timeframe: str = '5m', days: int = 30) -> str:
        ...
        output_path = (
            Path("runtime") / "training_data" / f"{symbol.replace('/', '_')}_{timeframe}.csv"
        )
