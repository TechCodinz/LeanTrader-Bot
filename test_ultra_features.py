...
def test_ultra_features():
    ...
    ultra = get_ultra_market_data(
        'kraken', enable_ml=False, enable_news=False, enable_onchain=False
    )
    ...
    try:
        ...
        momentum_indicators = [
            c for c in indicator_cols if 'rsi' in c or 'macd' in c or 'stoch' in c
        ]
        volatility_indicators = [
            c for c in indicator_cols if 'bb_' in c or 'atr' in c or 'volatility' in c
        ]
        microstructure_indicators = [
            c
            for c in indicator_cols
            if 'trend' in c or 'fvg' in c or 'liquidity' in c or 'session' in c
        ]

        print(
            f"   Moving Averages: {len(ma_indicators)} ({ma_indicators[:3]}{'...' if len(ma_indicators) > 3 else ''})"
        )
        print(
            f"   Momentum: {len(momentum_indicators)} ({momentum_indicators[:3]}{'...' if len(momentum_indicators) > 3 else ''})"
        )
        print(
            f"   Volatility: {len(volatility_indicators)} ({volatility_indicators[:3]}{'...' if len(volatility_indicators) > 3 else ''})"
        )
        print(
            f"   Microstructure: {len(microstructure_indicators)} ({microstructure_indicators[:3]}{'...' if len(microstructure_indicators) > 3 else ''})"
        )
        print()
        ...
        print(
            f"   RSI(14): {latest.get('rsi_14', 'N/A'):.2f}"
            if 'rsi_14' in latest
            else "   RSI(14): N/A"
        )
        print(f"   MACD: {latest.get('macd', 'N/A'):.4f}" if 'macd' in latest else "   MACD: N/A")
        print(
            f"   BB%: {latest.get('bb_percent', 'N/A'):.2f}"
            if 'bb_percent' in latest
            else "   BB%: N/A"
        )
        print(
            f"   Trend: {latest.get('trend_structure', 'N/A')}"
            if 'trend_structure' in latest
            else "   Trend: N/A"
        )
        print(
            f"   Session: {latest.get('session', 'N/A')}"
