                ...
                for symbol in self.crypto_pairs:
                    ...
                    if confidence >= 70 and signal != "HOLD":
                        ...
                        if trade_result:
                            ...
                            position_size = self.position_sizes.get(symbol, 0.001)
                            profit = abs(
                                price * position_size * (confidence / 100) * 0.02
                            )  # 2% profit factor
...
⏰ {datetime.now().strftime('%H:%M:%S')}"""
                                ...
                                print(
                                    f"🚀 GATE.IO {symbol}: {signal} @ ${price:.4f} | Profit: ${profit:.2f}"
                                )
...
⏰ {datetime.now().strftime('%H:%M:%S')}"""
                ...
                print(
                    f"🔄 Gate.io profit cycle completed - Trades: {trade_count}, Profit: ${self.total_profit:.2f}"
                )
