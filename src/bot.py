...
class TradingBot:
    ...
    async def _process_signal(self, signal: TradeSignal):
        ...
        try:
            logger.info(
                f"📈 Processing signal: {signal.symbol} {signal.action} (confidence: {signal.confidence:.2f})"
            )

            # Check if signal meets minimum confidence
            if signal.confidence < self.config['min_confidence']:
                logger.info(
                    f"⚠️ Signal confidence too low: {signal.confidence:.2f} < {self.config['min_confidence']}"
                )
                return

            # Risk management check (skip for SELL, allow closing)
    ...
    async def _execute_buy(self, signal: TradeSignal):
        ...
        try:
            ...
            if self.config['trading_enabled'] and self.active_exchange:
                order = await self.active_exchange.create_market_buy_order(signal.symbol, quantity)
    ...
    async def _execute_sell(self, signal: TradeSignal):
        ...
        try:
            ...
            # Save to database (if awaitable; else call directly)
            try:
                res = self.database.save_trade(order, signal)
                if callable(getattr(res, "__await__", None)):
                    await res
            except Exception:
                try:
                    await self.database.save_trade(order, signal)  # type: ignore
                except Exception:
                    pass
            try:
                res2 = self.database.update_position_pnl(signal.symbol, pnl)
                if callable(getattr(res2, "__await__", None)):
                    await res2
            except Exception:
                try:
                    await self.database.update_position_pnl(signal.symbol, pnl)  # type: ignore
                except Exception:
                    pass
