...
class Database:
    ...
    async def get_historical_data(
        ...
        try:
            ...
            if rows:
                data = pd.DataFrame(
                    rows, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'source']
                )
    ...
    async def save_portfolio_value(
        self, portfolio_value: float, available_capital: float, total_exposure: float
    ):
    ...
    async def cleanup_old_data(self, days_to_keep: int = 30):
        ...
        try:
            ...
            logger.info(
                f"🧹 Cleaned up {deleted_market} market data records and {deleted_status} status records"
            )
    ...
    async def close(self):
        """Close database connection (async API)."""
        if self.connection:
            try:
    ...
    def close_sync(self):
        """Close database connection (sync API for destructors/tests)."""
    ...
    def __del__(self):
        # Best-effort close to avoid file locks on Windows during test teardown
        try:
            self.close_sync()
        except Exception:
            pass
