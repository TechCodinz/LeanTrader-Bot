...
def eval_condition(expr: str, df: pd.DataFrame, reg):
    ...
    left_series = reg[left](df) if left in reg else df[left]
    try:
        r = reg[right](df) if right in reg else float(right)
    except Exception:
        r = df[right]
    if op == ">":
        return (left_series > r).astype(int)
    if op == "<":
        return (left_series < r).astype(int)
    if op == ">=":
        return (left_series >= r).astype(int)
...
def compile_strategy(spec: StrategySpec):
    ...
    def strategy_fn(frames):
        ...
        for rule in spec.signals:
            entry_hits = [
                eval_condition(c.feature, frames[c.timeframe], reg)
            ...
            outs.append(
                pd.DataFrame({"signal": rule.name, "side": rule.side, "go": final}, index=df.index)
            )
        res = (
