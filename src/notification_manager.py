...
class NotificationManager:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            'webhook': {'url': os.getenv('WEBHOOK_URL', ''), 'enabled': False},
        }
    ...
    async def _check_email_config(self):
        ...
        if (
            ...
            and self.config['email']['password']
            and self.config['email']['recipient']
        ):
            try:
    ...
    async def _check_sms_config(self):
        ...
        if (
            ...
            and self.config['sms']['auth_token']
            and self.config['sms']['from_number']
            and self.config['sms']['to_number']
        ):
            try:
                # Test Twilio connection (lazy import)
                ...
                _ = client.api.accounts(self.config['sms']['account_sid']).fetch()
        ...
        if self.config['webhook']['url']:
            try:
                ...
                response = requests.post(
                    self.config['webhook']['url'], json=test_payload, timeout=10
                )
    ...
    async def send_notification(
        self, title: str, message: str, level: str = 'INFO', channels: Optional[List[str]] = None
    ):
        ...
        try:
            ...
            if 'webhook' in channels and self.webhook_enabled:
                tasks.append(self._send_webhook(formatted_title, message, level))
            ...
            if tasks:
                ...
                # Fallback to console so tests can assert a call occurred
    ...
    async def _send_email(self, title: str, message: str):
        ...
        try:
            ...
            text = msg.as_string()
            server.sendmail(
                self.config['email']['username'], self.config['email']['recipient'], text
            )
            server.quit()
        ...
        return {
            'channels': {
                'telegram': {
                    ...
                    'configured': bool(
                        self.config['telegram']['bot_token'] and self.config['telegram']['chat_id']
                    ),
                },
                'email': {
                    'enabled': self.email_enabled,
                    'configured': bool(
                        self.config['email']['username'] and self.config['email']['password']
                    ),
                },
                'sms': {
                    'enabled': self.sms_enabled,
                    'configured': bool(
                        self.config['sms']['account_sid'] and self.config['sms']['auth_token']
                    ),
                },
                'webhook': {
