...
class RiskManager:
    ...
    def __init__(self, database):
        ...
        self.limits = RiskLimits(
            # Treat as fraction of portfolio; tests expect default 10%
        ...
        self.risk_history = []
        self.alert_levels = {'low': 0.5, 'medium': 0.7, 'high': 0.9, 'critical': 1.0}
    ...
    async def calculate_risk_metrics(
        ...
        try:
            ...
            # Maximum drawdown
            max_drawdown = (
                await self._calculate_max_drawdown(historical_data)
    ...
    async def _calculate_correlation_risk(self, positions: Dict) -> float:
        ...
        try:
            ...
            max_correlation = np.max(
                np.abs(correlation_matrix[np.triu_indices_from(correlation_matrix, k=1)])
            )
    ...
    async def can_open_position(self, symbol: str, position_value: float) -> bool:
        ...
        try:
            ...
            # Check position size limit as fraction of portfolio
            portfolio_value = (
                (self.current_metrics.portfolio_value if self.current_metrics else None)
            )
            ...
            max_value = float(self.limits.max_position_size) * float(portfolio_value)
            if position_value > max_value:
                logger.warning(
                    f"🚫 Position size exceeds limit: {position_value:.2f} > {max_value:.2f}"
                )
                return False

            # Check total exposure limit (fractions)
            current_exposure_frac = (
                float(self.current_metrics.total_exposure) if self.current_metrics else 0.0
            )
            add_exposure_frac = float(position_value) / max(1e-9, float(portfolio_value))
            new_exposure_frac = current_exposure_frac + add_exposure_frac

            if new_exposure_frac > self.limits.max_total_exposure:
                logger.warning(
                    f"🚫 Total exposure would exceed limit: {new_exposure_frac:.2f} > {self.limits.max_total_exposure}"
                )
                return False

            # Check concentration limit (fraction of portfolio)
            concentration_frac = float(position_value) / max(1e-9, float(portfolio_value))
            if concentration_frac > self.limits.max_concentration:
                logger.warning(
                    f"🚫 Position concentration exceeds limit: {concentration_frac:.2f} > {self.limits.max_concentration}"
                )
                return False
        ...
        try:
            ...
            if self.current_metrics.total_exposure > self.limits.max_total_exposure:
                risk_violations.append(
                    f"Total exposure: {self.current_metrics.total_exposure:.2%} > {self.limits.max_total_exposure:.2%}"
                )

            # Check drawdown limit
            if self.current_metrics.max_drawdown > self.limits.max_drawdown:
                risk_violations.append(
                    f"Max drawdown: {self.current_metrics.max_drawdown:.2%} > {self.limits.max_drawdown:.2%}"
                )

            # Check VaR limit
            if self.current_metrics.var_95 > self.limits.max_var:
                risk_violations.append(
                    f"VaR 95%: {self.current_metrics.var_95:.2%} > {self.limits.max_var:.2%}"
                )

            # Check concentration limit
            if self.current_metrics.concentration_risk > self.limits.max_concentration:
                risk_violations.append(
                    f"Concentration: {self.current_metrics.concentration_risk:.2%} > {self.limits.max_concentration:.2%}"
                )

            # Check leverage limit
            if self.current_metrics.leverage_risk > self.limits.max_leverage:
                risk_violations.append(
                    f"Leverage: {self.current_metrics.leverage_risk:.2%} > {self.limits.max_leverage:.2%}"
                )
            ...
            if risk_violations:
                ...
                # Log risk metrics
                logger.debug(
                    f"📊 Risk metrics: Exposure={self.current_metrics.total_exposure:.2%}, "
    ...
    async def _close_largest_positions(self, positions: Dict, close_pct: float = 0.5):
        ...
        try:
            ...
            sorted_positions = sorted(
                positions.items(), key=lambda x: x[1].size * x[1].current_price, reverse=True
            )
    ...
    async def calculate_position_size(
        self, symbol: str, confidence: float, volatility: float, portfolio_value: float
    ) -> float:
    ...
    async def get_risk_report(self) -> Dict:
        ...
        try:
            return {
                'current_metrics': {
                    'portfolio_value': (
                        self.current_metrics.portfolio_value if self.current_metrics else 0
                    ),
                    'total_exposure': (
                        self.current_metrics.total_exposure if self.current_metrics else 0
                    ),
                    'max_drawdown': (
                        self.current_metrics.max_drawdown if self.current_metrics else 0
                    ),
                    'var_95': self.current_metrics.var_95 if self.current_metrics else 0,
                    'sharpe_ratio': (
                        self.current_metrics.sharpe_ratio if self.current_metrics else 0
                    ),
                    'volatility': self.current_metrics.volatility if self.current_metrics else 0,
                    'correlation_risk': (
                        self.current_metrics.correlation_risk if self.current_metrics else 0
                    ),
                    'concentration_risk': (
                        self.current_metrics.concentration_risk if self.current_metrics else 0
                    ),
                    'leverage_risk': (
                        self.current_metrics.leverage_risk if self.current_metrics else 0
                    ),
                },
                'risk_limits': {
    ...
    async def is_healthy(self) -> bool:
        ...
        try:
            ...
            if self.current_metrics:
                if (
                    self.current_metrics.total_exposure > 1.0  # More than 100% exposure
                    or self.current_metrics.max_drawdown > 0.5
                ):  # More than 50% drawdown
                    return False
