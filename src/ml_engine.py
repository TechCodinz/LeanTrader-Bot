...
class MLEngine:
    ...
    def __init__(self, database, risk_manager):
        ...
        # Scalers
        self.scalers = {'price_scaler': StandardScaler(), 'feature_scaler': RobustScaler()}
    ...
    async def start(self):
        ...
        await asyncio.gather(self._model_retrain_loop(), self._performance_monitoring_loop())
        ...
        try:
            ...
            for symbol in self.symbols:
                data = await self.database.get_historical_data(
                    symbol, limit=self.config['lookback_period']
                )
    ...
    async def _engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Technical indicators (robust to dtype)
            close_vals = data['close'].astype(float).values
            data['rsi'] = talib.RSI(close_vals, timeperiod=14)
            macd, macd_sig, macd_hist = talib.MACD(close_vals)
            data['macd'], data['macd_signal'], data['macd_hist'] = macd, macd_sig, macd_hist
            bb_u, bb_m, bb_l = talib.BBANDS(close_vals)
            data['bb_upper'], data['bb_middle'], data['bb_lower'] = bb_u, bb_m, bb_l
            data['sma_20'] = talib.SMA(close_vals, timeperiod=20)
            data['sma_50'] = talib.SMA(close_vals, timeperiod=50)
            data['ema_12'] = talib.EMA(close_vals, timeperiod=12)
            data['ema_26'] = talib.EMA(close_vals, timeperiod=26)

            # Volume features
            vol_vals = data['volume'].astype(float).values
            data['volume_sma'] = talib.SMA(vol_vals, timeperiod=20)
    ...
    async def _create_targets(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            # Future returns
            data['future_return'] = (
                data['close'].shift(-self.config['prediction_horizon']) / data['close'] - 1
            )
    ...
    async def _create_lstm_model(self, data: pd.DataFrame):
        ...
        try:
            ...
            feature_columns = [
                ...
                if col not in ['symbol', 'timestamp', 'target', 'target_class', 'future_return']
            ]
            ...
            for symbol in data['symbol'].unique():
                ...
                for i in range(self.config['feature_window'], len(symbol_data)):
                    sequence = symbol_data.iloc[i - self.config['feature_window'] : i][
                        feature_columns
                    ].values
            ...
            X_scaled = (
                ...
                .fit_transform(X.reshape(-1, X.shape[-1]))
                .reshape(X.shape)
            )
    ...
    async def _create_dummy_models(self):
        ...
        dummy_lstm = Sequential(
            [Dense(10, activation='relu', input_shape=(10,)), Dense(3, activation='softmax')]
        )
        ...
        try:
            for symbol, data in market_data.items():
                ...
                if len(predictions) > 1:
                    final_prediction, final_confidence = await self._ensemble_predict(
    ...
    async def _predict_ensemble(self, features: np.ndarray, model_name: str) -> Tuple[int, float]:
        ...
        try:
            ...
            # Smooth confidence to avoid jitter in execution/risk sizing
            raw_conf = float(np.max(probabilities[0]))
            prev = self.model_performance.get(model_name, {}).get("last_conf", 0.5)
            alpha = float(os.getenv("CONF_SMOOTH_ALPHA", "0.7"))  # heavier weight on history
            confidence = float(alpha * prev + (1 - alpha) * raw_conf)
            self.model_performance.setdefault(model_name, {})["last_conf"] = confidence
    ...
    async def _prediction_to_signal(
        self, symbol: str, prediction: int, confidence: float, data: pd.DataFrame
...
# current_volume not used; omit to reduce noise

            # Calculate position size based on confidence and risk
            base_size = float(os.getenv('INITIAL_CAPITAL', 10000)) * float(
                os.getenv('POSITION_SIZE_PCT', 0.1)
            )
        ...
        while self.running:
            try:
                ...
                if (
                    ...
                    or (datetime.now() - self.last_retrain).total_seconds()
                    > self.config['retrain_hours'] * 3600
                ):
