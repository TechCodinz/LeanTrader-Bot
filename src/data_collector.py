...
class DataCollector:
    ...
    def __init__(self, database):
        ...
        self.config = {
            ...
            'max_retries': 3,
            'retry_delay': 5,
            # Aggregation toggles (env overridable)
            'enable_exchanges': self._env_bool('ENABLE_EXCHANGES', True),
            'enable_yfinance': self._env_bool('ENABLE_YF', True),
            'enable_websocket': self._env_bool('ENABLE_WS', True),
            'enable_orderbook': self._env_bool('ENABLE_ORDERBOOK', False),
            'enable_funding': self._env_bool('ENABLE_FUNDING', False),
        }
    ...
    def _env_bool(key: str, default: bool = False) -> bool:
        """Read boolean environment variable with typical truthy strings.

        Accepts 1/true/yes/on (case-insensitive). Returns default when unset.
        """
        val = os.getenv(key)
        if val is None:
            return bool(default)
    ...
    async def initialize(self):
        ...
        try:
            ...
            if self.config['enable_exchanges']:
                await self._initialize_exchanges()
            else:
                logger.info("ℹ️ Exchange initialization disabled by config")
    ...
    async def _initialize_exchanges(self):
        ...
        if self.config['enable_yfinance']:
            ...
            logger.info("ℹ️ Yahoo Finance fallback disabled by config")
            ...
            for exchange_name, exchange in (self.exchanges.items() if self.config['enable_exchanges'] else []):
                try:
                    ...
                    if ticker:
                        ...
                        # Optional enrichments: orderbook/funding (safe stubs)
                        if self.config['enable_orderbook']:
                            try:
                                ob = await self._fetch_orderbook_topn(exchange, symbol, topn=10)
                                # attach as optional columns if desired in the future
                                _ = ob
                            except Exception:
                                pass
                        if self.config['enable_funding']:
                            try:
                                fd = await self._fetch_funding_rate(exchange, symbol)
                                _ = fd
                            except Exception:
                                pass
                        ...
                        if symbol in self.realtime_data:
                            self.realtime_data[symbol] = pd.concat(
                                [self.realtime_data[symbol], data_row], ignore_index=True
                            )
                        ...
                        if symbol in self.historical_data:
                            self.historical_data[symbol] = pd.concat(
                                [self.historical_data[symbol], data_row], ignore_index=True
                            )
    ...
    async def _initialize_websockets(self):
        ...
        try:
            # Bybit WebSocket
            if self.config['enable_websocket'] and 'bybit' in self.exchanges:
    ...
    async def _fetch_orderbook_topn(self, exchange, symbol: str, topn: int = 10) -> Dict:
        """Fetch top-N orderbook levels (best-effort). Returns {'bids': [...], 'asks': [...]}.
        Safe no-op fallback when not available.
        """
        try:
            if hasattr(exchange, 'fetch_order_book'):
                ob = await exchange.fetch_order_book(symbol, limit=topn)
                bids = [[float(p), float(a)] for p, a in ob.get('bids', [])[:topn]]
                asks = [[float(p), float(a)] for p, a in ob.get('asks', [])[:topn]]
                return {'bids': bids, 'asks': asks}
        except Exception:
            pass
        return {'bids': [], 'asks': []}

    async def _fetch_funding_rate(self, exchange, symbol: str) -> Optional[float]:
        """Fetch perps funding rate if supported; returns None on failure.
        """
        try:
            if hasattr(exchange, 'fetchFundingRate'):
                fr = await exchange.fetchFundingRate(symbol)
                        ...
                        if new_data is not None and len(new_data) > 0:
                            ...
                            if symbol in self.historical_data:
                                self.historical_data[symbol] = (
                                    pd.concat(
                                        [self.historical_data[symbol], new_data], ignore_index=True
                                    )
                                ...
                                # Keep only last 5000 records
                                if len(self.historical_data[symbol]) > 5000:
                                    self.historical_data[symbol] = self.historical_data[
                                        symbol
                                    ].tail(5000)
                            else:
                ...
                if current_price:
                    ...
                    if historical_data is not None and len(historical_data) > 1:
                        ...
                        summary[symbol] = {
                            ...
                            'volume_24h': (
                                float(historical_data['volume'].sum())
