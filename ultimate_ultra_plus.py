...
class ExchangeConnector:
    ...
    async def execute_trade(self, symbol: str, side: str, size: float, leverage: int = 1) -> Dict:
        ...
        try:
            ...
            # Create order
            order = await self.bybit.create_market_order(symbol=symbol, side=side, amount=size)
...
class MoonSpotterEngine:
    ...
    async def scan(self) -> List[Dict]:
        ...
        try:
            ...
            # Check filters
            if (
                gem['price'] <= self.max_price
                and gem['liquidity'] >= self.min_liquidity
                and gem['age_hours'] < 24
            ):
...
class CryptoScalperEngine:
    ...
    def __init__(
        self, exchange: ExchangeConnector, deduplicator: SignalDeduplicator, db: DatabaseManager
    ):
            ...
            try:
                ...
                if model_path.exists():
                    ...
                    self.models[pair] = RandomForestRegressor(
                        n_estimators=100, max_depth=10, random_state=42, n_jobs=-1  # Use all cores
                    )
                    ...
                    if abs(sentiment) > 0.7:
                        ...
                        if symbol:
                            ...
                            if not self.deduplicator.is_duplicate(
                                ...
                                logger.info(
                                    f"News signal: {side.upper()} {symbol} (sentiment: {sentiment:.2f})"
                                )
...
class RiskManager:
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            'risk_level': (
                'HIGH'
                if abs(self.daily_drawdown) > 0.03
                else 'MEDIUM' if abs(self.daily_drawdown) > 0.01 else 'LOW'
            ),
        }
...
⚡ All engines running at full capacity!
        """.format(
            "TESTNET" if Config.USE_TESTNET else "LIVE", Config.MAX_SLOTS
        )
...
Price: {signal.get('price', 'Market')}
        ...
        try:
            await self.app.bot.send_message(
                chat_id=Config.TELEGRAM_CHAT_ID, text=msg, reply_markup=keyboard
            )
        except Exception as e:
...
Positions: {risk_status['open_positions']}/{risk_status['max_slots']}
        ...
        try:
            await self.app.bot.send_message(chat_id=Config.TELEGRAM_CHAT_ID, text=msg)
        except Exception as e:
            ...
            try:
                ...
                for opp in opportunities[:2]:  # Top 2
                    await self.telegram.send_signal(
                        ...
                        with_buttons=False,
                    )  # No execution buttons for arbitrage
