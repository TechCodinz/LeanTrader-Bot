...
from traders_core.connectors.crypto_ccxt import (market_buy, market_info,
                                                 market_sell, ohlcv_df,
                                                 ticker_price)
...
def decide_and_execute_crypto(
    ...
    reg = tag_regimes(
        df["close"], cfg["regimes"]["vol_window"], cfg["regimes"]["calm_quantile"]
    )
    ...
    try:
        ...
        METRICS.orders_total.labels(
            venue="crypto", symbol=symbol, status="live_sent"
        ).inc()
        if cfg.get("oco", {}).get("enabled", True):
        ...
        METRICS.orders_total.labels(
            venue="crypto", symbol=symbol, status="live_error"
        ).inc()
