    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str, limit: int = 400):
        ...
        try:
            ...
            for c in data.get("candles", []):
                ...
                low = float(c["mid"]["l"]) if c.get("mid") else 0.0
                cl = float(c["mid"]["c"]) if c.get("mid") else 0.0
                v = int(c.get("volume") or 0)
                rows.append([t, o, h, low, cl, v])
            return rows
    ...
    def price(self, symbol: str) -> float:
        try:
            r = pricing.PricingInfo(
                ...
                params={"instruments": self.normalize(symbol).replace("/", "_")},
            )
    ...
    def market_buy(self, symbol: str, units: float):
        try:
            data = {
                "order": {
                    "units": str(int(units)),
                    "instrument": self.normalize(symbol).replace("/", "_"),
                    "timeInForce": "FOK",
                    "type": "MARKET",
