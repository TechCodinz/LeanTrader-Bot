...
async def test_bot():
    ...
    try:
        ...
        if rsi_strategy:
            ...
            if signal:
                print(
                    f"✅ RSI Signal: {signal.signal_type.value} - confidence {signal.confidence:.2f}"
                )
            else:
    ...
    try:
        from core.risk_manager import OrderSide, OrderType
        ...
        print(
            f"✅ Risk Check: {'PASSED' if risk_check.allowed else 'FAILED'} - {risk_check.reason}"
        )
