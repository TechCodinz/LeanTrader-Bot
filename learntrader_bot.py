...
class LearntraderBot:
    ...
    def __init__(self):
        ...
        # Trading signals
        self.signals = {'crypto': {}, 'forex': {}, 'web3': {}, 'micro_moons': {}}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for coin in data:
                    ...
                    # Micro moon criteria
                    if (
                        market_cap < 10000000  # Under $10M market cap
                        and price_change > 20  # 20%+ price increase
                        and volume > 100000
                    ):  # Decent volume
                        ...
                        logger.info(
                            f"🌙 Micro Moon Spotted: {micro_moon['name']} ({micro_moon['symbol']}) - {micro_moon['change_24h']:.1f}%"
                        )
    ...
    async def trading_loop(self):
        ...
        while self.running:
            try:
                ...
                crypto_data = await self.fetch_market_data(
                    'crypto', self.crypto_pairs[:5]
                )  # Top 5 for demo
                ...
                for symbol, signal in crypto_signals.items():
                    if signal['strength'] > 0.6:
                        logger.info(
                            f"📈 CRYPTO: {symbol} | {signal['action']} | Strength: {signal['strength']:.1%} | Price: ${signal['price']:.2f}"
                        )
                        logger.info(f"   💡 Reasoning: {signal['reasoning']}")

                # 2. Analyze Forex Markets
                logger.info("💱 Analyzing Forex Markets...")
                forex_data = await self.fetch_market_data(
                    'forex', self.forex_pairs[:3]
                )  # Top 3 for demo
                ...
                for symbol, signal in forex_signals.items():
                    if signal['strength'] > 0.5:
                        logger.info(
                            f"💱 FOREX: {symbol} | {signal['action']} | Strength: {signal['strength']:.1%} | Price: {signal['price']:.4f}"
                        )
                        logger.info(f"   💡 Reasoning: {signal['reasoning']}")
                ...
                if micro_moons:
                    ...
                    for moon in micro_moons[:3]:  # Show top 3
                        logger.info(
                            f"   🚀 {moon['name']} ({moon['symbol']}) - {moon['change_24h']:.1f}% | MC: ${moon['market_cap']:,.0f}"
                        )
                ...
                for symbol, signal in web3_signals.items():
                    if signal['strength'] > 0.6:
                        logger.info(
                            f"🌐 WEB3: {symbol} | {signal['action']} | Strength: {signal['strength']:.1%} | Price: ${signal['price']:.2f}"
                        )
