...
if __name__ == "__main__":
    ...
    if not n.enabled:
        ...
        r = n._send(
            "Test message from LeanTrader smoke test at " + time.strftime("%Y-%m-%d %H:%M:%S")
        )
        if os.getenv("TELEGRAM_DEBUG", "false").lower() == "true":
            print("tg send response:", r)
        else:
            print(
                "Sent (fire-and-forget). If you do not receive a message, check token/chat id and bot permissions."
            )
