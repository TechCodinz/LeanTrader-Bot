...
class HiveCoordinator:
    ...
    def _consensus(self, fds: List[FrameDecision]) -> Tuple[Decision, List[str]]:
        ...
        cons = Decision(
            ...
            reasons=reasons[:6],
            notes="; ".join(rf"[{fd.timeframe}] {fd.dec.notes}" for fd in fds)[:240],
            votes=votes_all,
            features=feats_all,
        )
    ...
    def decide(
        self, symbol: str, is_fx: bool, fetch_ohlcv: Callable[[str, int], pd.DataFrame]
    ) -> Tuple[Decision, List[FrameDecision]]:
