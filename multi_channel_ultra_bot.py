...
class MultiChannelUltraTradingSystem:
    ...
    def __init__(self):
        ...
        self.mt5_config = {
            'broker': 'OctaFX',
            'account': '213640829',
            'password': __import__("os").getenv("MULTI_CHANNEL_ULTRA_BOT_PASSWORD", ""),
            'server': 'OctaFX-Demo',
            'connected': False,
        }
        ...
        self.channels = {
            'admin': telegram_config.get(
                'admin_id', '5329503447'
            ),  # Your personal chat (system updates only)
            'free': telegram_config.get('free_channel', '-1002930953007'),  # Free signal channel
            'vip': telegram_config.get(
                'vip_channel', '-1002983007302'
            ),  # VIP signal channel with trade buttons
        }
...
🔍 Spotted by ULTRA TRADING SYSTEM"""
                ...
                trade_buttons = self.create_trade_buttons(
                    'micro_moon', demo_micro_moon['symbol'], 'BUY', demo_micro_moon['price']
                )
                await self.send_vip_signal(signal, trade_buttons)
...
🚀 ULTRA TRADING SYSTEM"""
                    ...
                    trade_buttons = self.create_trade_buttons(
                        'forex', signal_data['pair'], signal_data['action'], signal_data['entry']
                    )
                    await self.send_vip_signal(signal, trade_buttons)
