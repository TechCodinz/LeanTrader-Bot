...
def score_and_pick(df: pd.DataFrame, min_usd_vol: float, w_liq: float, w_vol: float, max_symbols: int) -> pd.DataFrame:
    ...
    if df["liq_score"].max() > 0:
        df["liq_score"] /= df["liq_score"].max()
    else:
        df["liq_score"] = 0.0
...
def main():
    ...
    ap.add_argument(
        "--xp_days",
        type=int,
        default=int(os.getenv("UNIVERSE_XP_DAYS", "0")),
        help="Days of recent PnL to weight (0 = off)",
    )
    ap.add_argument(
        "--xp_weight",
        type=float,
        default=float(os.getenv("UNIVERSE_XP_WEIGHT", "0.25")),
        help="Blend weight 0..1",
    )
