...
class SwarmAgent:
    ...
    async def _analyze_black_swan(
        ...
        is_black_swan = (
            abs(recent_return) > volatility * 3  # 3-sigma event
            or volatility > returns.std() * 2  # High volatility
            or abs(recent_return) > 0.05  # 5%+ move
        )
...
class SwarmConsciousness:
    ...
    async def _process_signal_group(self, signals: List[SwarmSignal]):
        ...
        buy_confidence = (
            sum(s.confidence for s in buy_signals) / len(buy_signals) if buy_signals else 0
        )
        sell_confidence = (
            sum(s.confidence for s in sell_signals) / len(sell_signals) if sell_signals else 0
        )
        ...
        if consensus != 'hold':
            self.logger.info(
                f"Swarm consensus: {consensus} {signals[0].symbol} {signals[0].timeframe} "
    ...
    async def _handle_black_swan_event(self, signals: List[SwarmSignal]):
        ...
        self.logger.warning(
            f"🚨 BLACK SWAN EVENT DETECTED: {most_severe.symbol} "
    ...
    async def _update_collective_awareness(self, signals: List[SwarmSignal]):
        ...
        self.collective_awareness = CollectiveAwareness(
            ...
            risk_level=(
                'high'
                if market_state == 'black_swan'
                else 'medium' if market_state == 'volatile' else 'low'
            ),
        )
...
def integrate_swarm_consciousness(
