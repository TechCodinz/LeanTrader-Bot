    ...
    async def test_exchange(self, exchange_name, exchange):
        ...
        try:
            ...
            try:
                ...
                total_balance = sum(
                    [v['total'] for v in balance.values() if isinstance(v, dict) and 'total' in v]
                )
    ...
    def print_summary(self):
        ...
        for exchange_name, result in self.results.items():
            ...
            if status == 'SUCCESS':
                successful += 1
                logger.info(
                    f"✅ {exchange_name.upper():<10} | Markets: {result['markets']:>4} | BTC: ${result['btc_price']:>10,.2f} | Trading: {'Yes' if result['trading'] else 'No'}"
                )
            else:
                failed += 1
