...
class APITester:
        ...
        try:
            ...
            try:
                ...
                if orderbook and 'bids' in orderbook and 'asks' in orderbook:
                    ...
                    print(
                        f"  ✅ Orderbook fetched: {len(orderbook['bids'])} bids, {len(orderbook['asks'])} asks"
                    )
            except Exception as e:
            ...
            try:
                ...
                if balance:
                    ...
                    print("  ⚠️ No balance data (API keys may not be configured)")
            except Exception as e:
        ...
        for symbol in self.symbols:
            try:
                ...
                if symbol_opportunities:
                    print(
                        f"  ✅ Found {len(symbol_opportunities)} arbitrage opportunities for {symbol}"
                    )
                    for opp in symbol_opportunities:
                        print(
                            f"    💰 {opp['buy_exchange']} → {opp['sell_exchange']}: {opp['profit_percentage']:.4f}% profit"
                        )
                else:
    ...
    def print_summary(self, arbitrage_opportunities: List[Dict], scan_results: Dict):
        ...
        for exchange_name, result in self.test_results.items():
            status = (
                "✅ READY" if result['connected'] and result['ticker_fetched'] else "❌ NOT READY"
            )
        ...
        if arbitrage_opportunities:
            for opp in arbitrage_opportunities[:3]:  # Show first 3
                print(
                    f"  {opp['symbol']}: {opp['buy_exchange']} → {opp['sell_exchange']} ({opp['profit_percentage']:.4f}%)"
                )
        ...
        ready_exchanges = sum(
            ...
            if result['connected'] and result['ticker_fetched']
        )
        ...
        if ready_exchanges > 0:
            ...
            print("  ✅ Multi-platform scanning operational")
        else:
            print("  ❌ No exchanges ready")
            print("  ⚠️ System running in simulation mode")
            print("  💡 Configure API keys to enable real trading")

        print("\n🚀 Next Steps:")
        if ready_exchanges == 0:
            print("  1. Set up exchange accounts")
            print("  2. Configure API keys in api_config.json")
            print("  3. Run this test again")
            print("  4. Start paper trading")
        else:
            print("  1. Start paper trading: ./start_bot.sh")
            print("  2. Monitor performance: ./monitor_bot.sh")
