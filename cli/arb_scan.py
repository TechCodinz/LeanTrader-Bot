...
def main():
    ...
    p.add_argument(
        "--symbols", required=True, help="comma-separated symbols, e.g., BTC/USDT,ETH/USDT"
    )
    p.add_argument(
        "--venues", required=True, help="comma-separated venues, e.g., binance,kraken,coinbase"
    )
    p.add_argument("--min_bps", type=float, default=10.0)
    args = p.parse_args()

    symbols = [
        s.replace(" ", "").replace(",", "/") if "/" not in s else s for s in args.symbols.split(",")
    ]
