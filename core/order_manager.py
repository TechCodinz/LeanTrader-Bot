...
class OrderManager:
    ...
    async def create_order(
        ...
        **kwargs,
        ...
        if live and order.status == OrderStatus.OPEN:
            self._tracking_tasks[order.id] = asyncio.create_task(self._track_order(order.id))
                    ...
                    if fill_id not in self.fills:
                        fill = Fill(
                            ...
                            timestamp=datetime.fromtimestamp(
                                trade["timestamp"] / 1000, timezone.utc
                            ),
    ...
    async def _process_fill(self, fill: Fill):
        ...
        if order.filled_amount > 0:
            total_cost = sum(
                f.price * f.amount for f in self.fills.values() if f.order_id == order.id
            )
