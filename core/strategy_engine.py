...
class MACDStrategy(TechnicalStrategy):
    ...
    async def generate_signal(self, market_data: MarketData) -> Optional[Signal]:
        ...
        if current_macd > current_signal and macd_line.iloc[-2] <= signal_line.iloc[-2]:
            signal_type = SignalType.BUY
            confidence = (
                min(abs(current_histogram) / abs(current_macd), 1.0) if current_macd != 0 else 0.5
            )
            if confidence > 0.7:
                strength = SignalStrength.STRONG
            elif confidence > 0.5:
...
class StrategyEngine:
    ...
    async def run_strategies(self, symbol: str, timeframe: str = "1m") -> List[Signal]:
        ...
        if cooldown_key in self.last_signals:
            time_since_last = (
                datetime.now(timezone.utc) - self.last_signals[cooldown_key]
            ).total_seconds()
        ...
        try:
            ...
            market_data = MarketData(
                symbol=symbol, timeframe=timeframe, ohlcv=df, timestamp=datetime.now(timezone.utc)
            )
