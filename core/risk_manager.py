...
class RiskManager:
    ...
    async def check_order_risk(
        ...
        return RiskCheck(
            allowed=True, reason="All risk checks passed", risk_level=risk_level, warnings=warnings
        )
    ...
    def _check_total_exposure(
        self, symbol: str, amount: float, price: Optional[float]
    ) -> RiskCheck:
    ...
    def _check_var(self, symbol: str, amount: float, price: Optional[float]) -> RiskCheck:
        ...
        if abs(var_95 + position_impact) > self.portfolio_value * self.max_var:
            return RiskCheck(False, f"VaR would exceed limit {self.max_var:.2%}", RiskLevel.HIGH)
    ...
    def should_reduce_risk(self) -> bool:
        ...
        return (
            self.current_drawdown > self.max_drawdown * 0.8
            or len(self.positions) > 10
            or any(
                abs(pos.amount * pos.current_price) > self.portfolio_value * self.max_concentration
