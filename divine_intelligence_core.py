...
class DivineIntelligenceCore:
        ...
        try:
            ...
            if len(X) > 20 and len(y) > 20:
                ...
                if best_model is not None and best_score > 0.6:  # Minimum accuracy threshold
                    ...
                    logger.info(
                        f"🧠 Model trained for {symbol} {timeframe}: {best_model_name} (Accuracy: {best_score:.3f})"
                    )
    ...
    def enhance_winning_strategies(self, df):
        ...
        try:
            ...
            if len(high_confidence_trades) > 5:
                logger.info(
                    f"🧠 Enhancing winning strategies: {len(high_confidence_trades)} high-confidence wins"
                )
    ...
    def get_learning_stats(self):
        ...
        try:
            ...
            # Get model count
            cursor.execute(
                'SELECT COUNT(DISTINCT symbol || "_" || timeframe) FROM model_performance'
            )
