...
class CompleteEnhancedUltraTradingSystem:
    ...
    def __init__(self):
        ...
        # Market data cache
        self.market_data = {'crypto_prices': {}, 'forex_rates': {}, 'last_update': None}
    ...
    async def fetch_all_market_data(self):
        ...
        try:
            ...
            self.market_data.update({'crypto_prices': crypto_prices, 'last_update': datetime.now()})
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.market_awareness.update(
                        {
                            ...
                            'market_sentiment': (
                                'bullish'
                                if global_data['market_cap_change_percentage_24h_usd'] > 0
                                else 'bearish'
                            ),
                    ...
                    logger.info("🧠 Market Awareness Updated:")
                    logger.info(
                        f"💰 Total Market Cap: ${self.market_awareness['total_market_cap']:,.0f}"
                    )
                ...
                buttons = [
                    [
                        InlineKeyboardButton(
                            f"📈 {action} {symbol}",
                            callback_data=f"trade_crypto_{symbol}_{action}_{entry_price}",
                        ),
                    ...
                    [
                        InlineKeyboardButton(
                            "🚀 Execute on Bybit", callback_data=f"bybit_trade_{symbol}_{action}"
                        ),
    ...
    async def handle_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data.startswith('trade_crypto_'):
                ...
                # Execute trade
                await self.execute_bybit_trade(
                    symbol, action.lower(), 0.001
                )  # Small amount for test
            ...
            for signal in signals:
                if signal['confidence'] >= 80:  # VIP
                    trade_buttons = self.create_trade_buttons(
                        'crypto', signal['symbol'], signal['action'], signal['price']
                    )
    ...
    async def analyze_crypto_pair(self, pair, price, timeframe):
        ...
        try:
            ...
            if confidence >= 70:  # Only send high-confidence signals
                action = (
                    'BUY'
                    if macd_signal == 'BUY' and rsi < 70
                    else 'SELL' if macd_signal == 'SELL' and rsi > 30 else 'HOLD'
                )
