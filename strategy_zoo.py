...
def swing_flags(h: pd.Series, low: pd.Series, sw_n=3):
    ...
    sw_n = int(sw_n)
    hh = (
        h.rolling(2 * sw_n + 1, center=True)
        .apply(lambda x: float(x[sw_n] == x.max()), raw=False)
        .fillna(0)
        .astype(bool)
    )
    ll = (
        low.rolling(2 * sw_n + 1, center=True)
        .apply(lambda x: float(x[sw_n] == x.min()), raw=False)
        .fillna(0)
        .astype(bool)
    )
    return hh, ll
...
class NakedFXPriceAction(BaseStrategy):
    ...
    def apply(self, df: pd.DataFrame, **p) -> pd.DataFrame:
        ...
        for i in range(len(d)):
            if i == 0:
                pin_bull.append(False)
                eng_bull.append(False)
                continue
            pin_bull.append(
                bullish_pinbar(
                    o.iloc[i],
