    ...
    def initialize_database(self):
        ...
        try:
            ...
            tables = [
                ...
                (
                    'auto_trades',
                    'CREATE TABLE IF NOT EXISTS auto_trades (id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT NOT NULL, side TEXT NOT NULL, amount REAL NOT NULL, entry_price REAL NOT NULL, current_price REAL, unrealized_pnl REAL, status TEXT, tp1 REAL, tp2 REAL, tp3 REAL, stop_loss REAL, confidence REAL, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, exit_time DATETIME, exit_price REAL, final_pnl REAL)',
                ),
            ]
        ...
        try:
            ...
            if channel == 'vip' and signal_data:
                keyboard = [
                    ...
                    [
                        InlineKeyboardButton(
                            "📊 TP1", callback_data=f"tp1_{symbol}_{signal_data['tp1']:.2f}"
                        ),
                        InlineKeyboardButton(
                            "📊 TP2", callback_data=f"tp2_{symbol}_{signal_data['tp2']:.2f}"
                        ),
                        InlineKeyboardButton(
                            "📊 TP3", callback_data=f"tp3_{symbol}_{signal_data['tp3']:.2f}"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            "🛡️ SET SL", callback_data=f"sl_{symbol}_{signal_data['stop_loss']:.2f}"
                        ),
    ...
    async def execute_auto_trade(self, symbol, signal_data, price_data):
        ...
        try:
            ...
            order = await self.bybit.create_market_order(
                symbol=symbol, side=action.lower(), amount=amount, params={'timeInForce': 'IOC'}
            )
            ...
            logger.info(
                f"✅ AUTO TRADE EXECUTED: {symbol} {action} at ${current_price:.4f} (Confidence: {confidence:.1f}%)"
            )
            return True
...
🚀 AUTO TRADING BOT DOMINATING!"""
                    ...
                    if trade_executed:
                        logger.info(
                            f"🤖 AUTO TRADE: {pair} {signal['action']} at ${price_data['price']:.4f} (Confidence: {signal['confidence']:.1f}%)"
                        )
