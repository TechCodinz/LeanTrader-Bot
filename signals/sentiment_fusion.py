...
class FusionModel:
    ...
    weights: Dict[str, float] = field(
        default_factory=lambda: {"onchain": 1.0, "twitter": 0.7, "news": 0.5, "telegram": 0.6}
    )
    half_life_sec: int = 900

    def __post_init__(self):
        self._events: Deque[Tuple[int, str, float, float, str]] = (
            deque()
        )  # (ts, asset, score, conf, source)
    ...
    def features(self, when_ts: Optional[int], asset: str) -> Dict[str, float]:
        ...
        for ts, sym, sc, cf, src in reversed(self._events):
            ...
            buzz += 1.0 * w_src
        return {
            "sentiment": float(s_val),
            "sentiment_conf": float(min(1.0, s_conf)),
            "buzz": float(buzz),
        }
