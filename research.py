...
def simple_backtest_long_only(
    ...
    return {
        ...
        "pnl_avg": float(np.mean(trade_pnls) if trade_pnls else 0.0),
        "max_dd": max_dd,
        "sharpe": sharpe,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for step_idx, (tr_s, tr_e, te_s, te_e) in enumerate(windows, start=1):
            ...
            if len(tr) < 200 or len(te) < 100:
                continue
            d_tr = sig_fun(tr)
            simple_backtest_long_only(d_tr, fee_bps=fee_bps, slip_bps=slip_bps)
            d_te = sig_fun(te)
            m_te = simple_backtest_long_only(d_te, fee_bps=fee_bps, slip_bps=slip_bps)
            s = score_metrics(m_te)
            scores.append(s)
...
def optimize_symbol(
    ...
    for sname in REGISTRY.keys():
        ...
        if (best_overall is None) or (artifact["score"] > best_overall["score"]):
            ...
            ).to_csv(REPORTS / f"{symbol.replace('/', '_')}_{sname}_optuna.csv", index=False)
